...
# -*- coding: utf-8 -*-
"""
Created on Fri May 27 10:21:43 2022
...
import numpy as np
import flooddata as fd
import math

def predictLevel(x,y,preX):
    yt = [float(i) for i in y]
    res = np.polyfit(x, yt, 2, rcond=None, full=False, w=None, cov=False)
        ...
        for i in range(len(dem)):
            for j in range(len(dem[i,:])):
                if bnd[i,j] != np.nan and bnd[i,j] == w['id']:
                    if round(dem[i,j],3) < round(float(w['wlevel'][stime]),3):
                        outimg[i,j] = float(w['wlevel'][stime]) - demorigin[i,j]
                    else:
                        outimg[i,j] = -9
...
def genPredictedImage(data,stime, outfile):
    ...
    for w in data:
        ...
        for i in range(len(dem)):
            for j in range(len(dem[i,:])):
                if bnd[i,j] != np.nan and bnd[i,j] == w['id']:
                    if round(dem[i,j],3) < round(lev,3):
                    # if dem[i,j] <= w['wlevel'][11]-2:
                        outimg[i,j] = round(lev,2) - round(demorigin[i,j],2)
                    else:
                        outimg[i,j] = -9
...
ds = gdal.Open("/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Aug2022/data/bnd_brk_geog.tif")
...
ds = gdal.Open("/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Aug2022/data/demadjust_geo5.tif")
dem = np.array(ds.GetRasterBand(1).ReadAsArray())

ds = gdal.Open("/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Aug2022/data/dembrksm1_geog.tif")
demorigin = np.array(ds.GetRasterBand(1).ReadAsArray())
...
outFile = [
    {"time": 11, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood4.tif"},
    # {"time": 10, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood3.tif"},
    # {"time": 9, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood2.tif"},
    # {"time": 8, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood1.tif"},
    ]


outFile_predict = [
      {"time": 13, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood5.tif"},
      # {"time": 14, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood6.tif"},
      # {"time": 15, "outfile": "/Users/kampanartpiyathamrongchai/MyDoc/2564/project/irri/Jul/result/flood7.tif"},
